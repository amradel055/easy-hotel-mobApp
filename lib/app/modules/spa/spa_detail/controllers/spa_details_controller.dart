...
class SpaDetailsController extends GetxController {
  ...
  void onInit() async {
    if (id != null) {
      await getSpaDetailById();
    } else {
      await getSpaDetailByHotel();
    }
    calcTotalPrice();
    super.onInit();
  }

  Future getSpaDetailById() async {
    isLoading(true);
    ...
    SpaRepository().getSpaDetail(request,
        onSuccess: (data) {
          spa = data.data;
          (spa?.offersDTOList?.isEmpty ?? true)
              ? serviceIndex(0)
              : serviceIndex(2);
        },
  ...
  Future getSpaDetailByHotel() async {
    isLoading(true);
    final request = SpaDetailRequest(
  ...
  getOfferItems(int offerItemId, RxInt index) {
    if (!servicesSelected.toList().contains(offerItemId)) {
      ...
      servicesSelectedPrices.add(
          spa!.spaItemsDtoList![index.value].salePrice == null
              ? spa!.spaItemsDtoList![index.value].price!.toString()
              : spa!.spaItemsDtoList![index.value].salePrice.toString());
  ...
  calcTotalPrice() {
    ...
    selectedAdd.toList().forEach((addition) {
      totalPrice.value += (addition.salePrice != null && addition.salePrice != 0
              ? addition.salePrice
              : addition.price)!
          .toDouble();
    });
  ...
  changeTime(TimeOfDay value) {
    value.hour <= 10 && value.minute <= 10
        ? timeController.value.text = "0${value.hour}:0${value.minute}"
        : value.hour <= 10 && value.minute >= 10
            ? timeController.value.text = "0${value.hour}:${value.minute}"
            : value.hour >= 10 && value.minute <= 10
                ? timeController.value.text = "${value.hour}:0${value.minute}"
